...
public:
    ...
    bool book(int start, int end) {
        auto it = events.lower_bound({start, end});//it->val >= val, insert after it.
        if(it!= events.end() && it->first < end) return false;// overlap with next pos.
        if(it!= events.begin() && start < (--it)->second) return false;//overlap with prev.
        //no overlap, insert.
